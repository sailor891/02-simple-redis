...
impl RespDecode for SimpleString {
    const PREFIX: &'static str = "+";
    fn decode(buf: &mut BytesMut) -> Result<Self, RespError> {
        let end = extract_simple_frame_data(buf.as_ref(), Self::PREFIX)?;
        // data获取buf中"+{}\r\n"这段数据 buf[0,at)，数据的所有权转移到data
...
impl RespDecode for BulkString {
    const PREFIX: &'static str = "$";
    fn decode(buf: &mut BytesMut) -> Result<Self, RespError> {
        // end:第一部分${d}的长度，len:第二部分，String元素数据长度（d)
        let (end, len) = parse_length(buf.as_ref(), Self::PREFIX)?;
        // 元素主题部分
        let remain = &buf[end + CRLF_LEN..];
        if remain.len() < len + CRLF_LEN {
...
impl RespDecode for RespArray {
    const PREFIX: &'static str = "*";
    fn decode(buf: &mut BytesMut) -> Result<Self, RespError> {
        // end:第一部分${d}的长度，len:第二部分，数组元素个数（d)
        let (end, len) = parse_length(buf, Self::PREFIX)?;
        // 计算符合RespFrame格式的元素的bs长度
